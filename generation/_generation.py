...
def generate_next_token(
    ...
    *,
    ...
    # print("logits: ", (logits[0]==logits[1]).all())
    return (
...
def generate(
    ...
    *,
    ...
    for i in range(max_generated_tokens - 1):
        ...
        # q = torch.ones(
        #     (1, model.tok_embeddings.num_embeddings), device=prompt.device
        # ).exponential_(1, generator=rng).expand(bsz, -1)
        ...
        #     print("tokens: ",tokens)
        #     print("logits: ", logits)
        #     print("current masks: ", curr_masks[0][0][:42])
        # else:
        #     exit()
        generated_tokens = torch.cat([generated_tokens, tokens], dim=-1)
        curr_pos += 1
    ...
    generated_logits = generated_logits[:, prompt_length:]
    ...
    if stop_tokens is not None:
        ...
        generated_tokens[stop_token_mask[:, prompt_length:] == 0] = pad_id
        # generated_tokens *= stop_token_mask[:, prompt_length:]
        generated_logits *= stop_token_mask[:, prompt_length:-1, None]
