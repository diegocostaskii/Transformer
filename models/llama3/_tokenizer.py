...
class Llama3Tokenizer(ModelTokenizer, Transform):
    ...
    def _tokenize_header(self, message: Message) -> List[int]:
        ...
        if message.role != "":
            return (
                self.encode("<|start_header_id|>" + message.role.strip() + "<|end_header_id|>\n\n", add_bos=False, add_eos=False)
            )
        else:
            return []  # Pre-training role set as ""
        # return (
        ...
        for item in message.content:
            if item["type"] == "text":
                tokenized_body += self.encode(
                    item["content"].strip()+"<|eot_id|>", add_bos=False, add_eos=False
                )
    ...
    def tokenize_message(
        ...
        *,
        ...
        tokenized_body = self._tokenize_body(message) if add_end_tokens else []
        # tokenized_end = self._tokenize_end(message) 
