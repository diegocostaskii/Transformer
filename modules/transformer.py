...
class StackedTransformerDecoder(TransformerDecoder):
    def __init__(
        self,
        *args,
        num_stacks: int = 5,
    ...
    def forward(
        ...
        *,
        ...
        """
        rewrite forward with stacked Transformer
        """
        bsz, seq_len = tokens.shape
        ...
        #     if j in self.output_hidden_states:
        #         hidden.append(h)
        #     # shape: [b, s, d]
        #     h = layer(
        #         h,
        ...
        for i in range(self.num_stacks):
            ...
            # print("stack i: ", i)
            # temp_output = self.output(self.norm(h)).float()
            # print("temp size: ", temp_output.size())
            # print("select logits: ", torch.argmax(temp_output, dim=-1, keepdim=True)[-1][-1].to(dtype=torch.int))
            # print("max logits: ", torch.max(torch.nn.functional.softmax(temp_output[0][-1])))
